...
then emits input and expected-output hex files. Run `make npu-core-sim` from the
...
```text
PYTHONPATH=sw/tools python -m npu_phase0.cli emit-rtl-fixtures --arch arch/configs/npu_v0.jsonc --out-dir build/rtl_fixture
```

`make npu-core-sim` depends on `rtl-fixtures`, then passes the generated include
