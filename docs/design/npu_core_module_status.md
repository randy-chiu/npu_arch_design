...
| Uop scheduler / dispatcher | `docs/design/npu_core.md` | `hw/npu_core/rtl/scheduler/npu_v0_uop_scheduler.sv` | Integrated for Matrix, Scale/Mask, and Compiler-expanded Softmax primitive fetch, decode, issue, and completion wait | scheduler active/wait counters and semantic primitive issue/wait spans published in PPA | core, Transformer SoC, and Transformer PPA pass | add typed engine wait reasons and valid/ready command/response |
...
| Uop/primitive scheduler boundary | `arch/specs/transformer/v1/uop_isa_v1.md` | `hw/npu_core/rtl/scheduler/npu_v0_uop_scheduler.sv` | Common Matrix, Scale/Mask, and Compiler-expanded Softmax paths integrated; Compute cluster only routes one issued primitive | active/wait counters and row/lane issue semantics integrated | core, Transformer SoC, and Transformer PPA pass | generalized valid/ready response payloads, typed wait reasons, and loop-vs-expanded PPA decision remain |
